...
def full_coref_evaluation(
    ...
    log_file = path.join(log_dir, split + gold_ment_str + gold_str + tf_str + _iter + ".log.jsonl")
    ...
    with open(log_file, "w") as f:
        ...
        for example in data_iter_map[split][dataset]:
            ...
            # if(len(example["subtoken_map"]) > 3000):
            #     print("Sorry I skipped:", example)
            #     continue
            # print(example)
            start_time = time.time()
            pred_mentions,pred_mentions_emb,mention_scores, gt_actions, pred_actions, coref_scores, entity_cluster_states = model(example,teacher_force=teacher_force,gold_mentions=gold_mentions)
            # if config.log_vals: 
            #     GT_ACTIONS["tf" if teacher_force== True else "ntf" ][example["doc_key"]] = gt_actions
            #     COREF_SCORES["tf" if teacher_force== True else "ntf" ][example["doc_key"]] = coref_scores
            #     MENTIONS["tf" if teacher_force== True else "ntf"][example["doc_key"]] = pred_mentions
            ...
            # print("Len of Clusters: ",len(predicted_clusters))
            evaluator.update(
            ...
            # print("Keys of log example: ",log_example.keys())
        ...
        for indv_metric, indv_evaluator in zip(config.metrics, evaluator.evaluators):
            perf_str += (
                ", " + indv_metric + ": {}".format(indv_evaluator.get_f1() * 100)
            )
        ...
        # if config.log_vals:
        #     torch.save(COREF_SCORES["tf" if teacher_force== True else "ntf" ], f"tf_{teacher_force}_{dataset}_{split}_coref.pth")
        #     torch.save(MENTIONS["tf" if teacher_force== True else "ntf" ],f"tf_{teacher_force}_{dataset}_{split}_mentions.pth")
        #     with open(f"tf_{teacher_force}_{dataset}_{split}_gt.pickle",'wb') as fp:
        #         pickle.dump(GT_ACTIONS["tf" if teacher_force== True else "ntf" ],fp)
        ...
        try:
            ...
            if final_eval and is_canonical and path_exists_bool:
                ...
                for indv_metric in config.metrics:
                    result_dict[indv_metric]["recall"] = conll_results[indv_metric.lower()]["r"]
                    result_dict[indv_metric]["precision"] = conll_results[indv_metric.lower()]["p"]
                    result_dict[indv_metric]["fscore"] = conll_results[indv_metric.lower()]["f"]

                average_f1 = sum(
                    results["f"] for results in conll_results.values()
                ) / len(conll_results)
                result_dict["fscore"] = average_f1
...
### Might present logging issues if logged For now letting it is.
def targeted_coref_evaluation(
    ...
    with open(log_file, "w") as f:
        ...
        for document in data_iter_map[split][dataset]:
            pred_mentions,pred_mentions_emb,mention_scores, gt_actions, pred_actions,coref_scores = model(document)
