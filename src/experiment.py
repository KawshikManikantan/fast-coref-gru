...
class Experiment:
    ...
    def agg(self,datadepdict):
        ...
        agg_dict["loss_norm"] = agg_dict["total"]/agg_dict["mention_count"]
    ...
    def _wandb_log(self,split,stat_per_dataset,agg_stat,coref_dict,step=None):
        for dataset_name in stat_per_dataset:
            ...
            if stat_per_dataset[dataset_name]["mention_count"] > 0.0:    
                wandb.log(
                            data={
                                    f"{split}/{dataset_name}/loss_norm": stat_per_dataset[dataset_name]["total"]/stat_per_dataset[dataset_name]["mention_count"]
                                },
                            step = step,
                        )
            else:
