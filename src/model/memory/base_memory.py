...
class BaseMemory(nn.Module):
    ...
    def __init__(self, config: DictConfig, span_emb_size: int, drop_module: nn.Module):
        ...
        # self.input_size = num_embs * self.mem_size + config.num_feats * config.emb_size
        ...
        if config.entity_rep == "gru":
            self.gru = nn.GRU(self.mem_size, self.mem_size)
    ...
    def initialize_memory(
        ...
        **kwargs
        ...
        return mem, ent_counter, last_mention_start,first_mention_start
    ...
    def coref_update(
        ...
        elif self.config.entity_rep == "gru":
            print("GRU")
            hidden_vec  = torch.unsqueeze(mem_vectors[cell_idx],0)
            x_vec = torch.unsqueeze(ment_emb,0)
            coref_vec = torch.squeeze(self.gru(x_vec,hidden_vec)[0])
        else:
