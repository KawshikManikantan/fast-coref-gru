...
class MentionProposalModule(nn.Module):
    ...
    def __init__(self, config, train_config,drop_module=None):
        ...
        print("Whats this hidden size?", self.doc_encoder.hidden_size)
    ...
    def get_span_embeddings(
        self, document, encoded_doc: Tensor, ment_starts: Tensor, ment_ends: Tensor
    ) -> Tensor:
        ...
        # print("Tokenizer:",self.doc_encoder.tokenizer)
        # print("Length of tensorized sentence: ",document["tensorized_sent"][0].shape)
        # print("Length of Encoded Document: ", encoded_doc.shape)
        # print("Single lengthed mentions:",ment_starts[ment_starts == ment_ends])
        # print((ment_starts+1)[:].shape)
        # print("Mention Tokens:", self.doc_encoder.tokenizer.convert_ids_to_tokens([document["tensorized_sent"][0][ment+1] for ment in ment_starts[ment_starts == ment_ends]]))
        # print("Mention Decode:", self.doc_encoder.tokenizer.decode([document["tensorized_sent"][0][ment+1] for ment in ment_starts[ment_starts == ment_ends]]))
        ...
        if self.config.mention_params.ment_emb == "attn":
            ...
            # print("Encoded Doc dtype: ", encoded_doc.dtype)
            # print("Attention: ", self.mention_attn.dtype)
            word_attn = torch.squeeze(self.mention_attn(encoded_doc), dim=1)  # [T]
            # print("Length of Attention: ", word_attn.shape)
    ...
    def pred_mentions(
        ...
        span_embs = self.get_span_embeddings(document,encoded_doc, cand_starts, cand_ends)
        # print("Is this the culprit?",span_embs.shape)
    ...
    def forward(self, document: Dict,eval_loss=False,gold_mentions=False) -> Dict:
        ...
        # document_masked = self.mask_doc(document)
        ...
        # gold_dict: Dict = self.transform_gold_mentions(document)

        pred_starts: Tensor = output_dict["ment_starts"]
        pred_ends: Tensor = output_dict["ment_ends"]
        
        # gold_starts: Tensor = gold_dict["ment_starts"]
        # gold_ends: Tensor = gold_dict["ment_ends"]

        # Stack the starts and ends to get the mention tuple
        output_dict["ments"] = torch.stack((pred_starts, pred_ends), dim=1)
        # gold_dict["ments"] = torch.stack((gold_starts, gold_ends), dim=1)
        
        # Get mention embeddings
        mention_embs: Tensor = self.get_span_embeddings(
            document,encoded_doc, pred_starts, pred_ends
        )
        
        # gold_mention_embs: Tensor = self.get_span_embeddings(
        #     encoded_doc, gold_starts, gold_ends
        # )
        
        output_dict["ment_emb_list"] = torch.unbind(mention_embs, dim=0)
        # gold_dict["ment_emb_list"] = torch.unbind(gold_mention_embs, dim=0)
